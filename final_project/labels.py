...
# Main types of messages to be predicted
message_type_classes = ['building relationships', 'destroying relationships', 'defense', 'identifying actions',
                        'identifying characteristics', 'clarification questions', 'explanations for bad behavior',
                        'short responses']

# For each main message type, create labels for the submessage types
classes, sub_class_label = {}, len(message_type_classes)

for i, message_type in enumerate(message_type_classes):
    classes[message_type] = {'label': i, 'subclasses': {}}
    sub_classes = all_classes[i]

    for sub_class in sub_classes:
        classes[message_type]['subclasses'][sub_class] = sub_class_label
        sub_class_label += 1

# for key, vals in classes.items():
#     print(f'{key}, {vals["label"]}:')

#     for sub_message_key, sub_message_val in vals['subclasses'].items():
#         print(f'{sub_message_key}, {sub_message_val}')
#     print()
